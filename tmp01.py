...
def train(args):
    ...
    a = Conv2D(n_outfmaps=64, n_row=3, n_col=5, act='linear', strides=(1, 1), border_mode=(1, 2))(a)
    a = BN(axis=(0, 2, 3))(a)
    a = Activation('relu')(a)
    a = Conv2D(n_outfmaps=64, n_row=3, n_col=5, act='linear', strides=(1, 1), border_mode=(1, 2))(a)
    a = BN(axis=(0, 2, 3))(a)
    ...
    a = Conv2D(n_outfmaps=n_out, n_row=1, n_col=1, act='sigmoid', border_mode=(0,0), name='seg_masks')(a)
    
    weight1d = np.power(r * np.ones(240*64), np.arange(240*64))
    a8 = Lambda(_global_rank_pooling, weight1d=weight1d, name='a8')(a)
...
def print_stats(at_pds, at_gts, sed_pds, sed_gts):
    ...
    logging.info("%sPrec\tRecall\tFvalue\tER\tn_sub\tn_del\tn_ins" % "".ljust(16))
    ...
    logging.info("%s*%.3f\t*%.3f\t*%.3f\t*%.3f\t*%.3f\t*%.3f\t*%.3f" % ("*Global".ljust(16), prec, recall, fvalue, er, n_sub, n_del, n_ins))
    ...
    logging.info("%s*%.3f\t*%.3f\t*%.3f\t*%.3f\t*%.3f\t*%.3f\t*%.3f" % ("*Avg. of each".ljust(16), prec, recall, fvalue, er, n_sub, n_del, n_ins))
    ...
    for i1 in xrange(n_classes):
        logging.info("%s%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f" % (events[i1].ljust(16), precs[i1], recalls[i1], fvalues[i1], ers[i1], n_subs[i1], n_dels[i1], n_inss[i1]))
